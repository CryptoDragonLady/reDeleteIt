...
type UITimeUnit = "minutes" | "hours" | "days" | "months";
type PersistenceMode = "session" | "local";

interface UIDeleteConfig {
  mode: UIDeleteMode;
  thresholdMs: number;
  summary: string;
  dryRun: boolean;
...
interface UIConfig extends UIDeleteConfig {
...
window.addEventListener("error", (event) => {
  console.warn("redDeleteIt UI: ignored error", (event as ErrorEvent).error || event.message);
}, true);
...
function openModal(): void {
  ...
  const closeBtn = modal.querySelector("#rdi-close") as HTMLButtonElement;
  const modeInputs = Array.from(modal.querySelectorAll<HTMLInputElement>("input[name='rdi-mode']"));
  const ageRow = modal.querySelector("#rdi-age-row") as HTMLDivElement;
  const amountInput = modal.querySelector("#rdi-amount") as HTMLInputElement;
  const unitSelect = modal.querySelector("#rdi-unit") as HTMLSelectElement;
  const loadSubsBtn = modal.querySelector("#rdi-load-subs") as HTMLButtonElement;
  const loadStatus = modal.querySelector("#rdi-load-status") as HTMLSpanElement;
  const subsSelect = modal.querySelector("#rdi-subs") as HTMLSelectElement;
  const nsfwCheckbox = modal.querySelector("#rdi-nsfw") as HTMLInputElement;
  const dryRunCheckbox = modal.querySelector("#rdi-dryrun") as HTMLInputElement;
  const persistSelect = modal.querySelector("#rdi-persist") as HTMLSelectElement;
  const startBtn = modal.querySelector("#rdi-start") as HTMLButtonElement;

  // populate from saved
  (modeInputs.find(i => i.value === saved.mode) ?? modeInputs[0]).checked = true;
  amountInput.value = saved.thresholdMs ? String(saved.thresholdMs / unitToMsUI(unitSelect.value as UITimeUnit)) : "30";
  ...
  startBtn.addEventListener("click", () => {
    const mode = (modeInputs.find(i => i.checked)?.value ?? "all") as UIDeleteMode;
    const unit = unitSelect.value as UITimeUnit;
    ...
    if (nsfwCheckbox.checked) summaryParts.push("NSFW only");
    if (dryRunCheckbox.checked) summaryParts.push("dry run");
    ...
    const config: UIConfig = {
      ...
      persistence: persistSelect.value as PersistenceMode,
      runOnLoad: true,
    };
    saveConfigUI(config);
...
function populateSubs(select: HTMLSelectElement, subs: string[], selected: string[]): void {
...
function getCSRFTokenUI(): string | null {
...
async function deleteNextUI(config: UIConfig, token: string): Promise<boolean> {
...
function findNextTargetUI(config: UIConfig): { element: Element, operation: string, input: Object } | null {
...
function buildTargetUI(element: Element): { element: Element, operation: string, input: Object } | null {
...
function extractTimestampUI(element: Element): number | null {
  const timeElement = element.querySelector("time") as HTMLTimeElement | null;
...
function parseTimestampUI(value: string | null | undefined): number | null {
  if (!value) {
    return null;
  }
  if (/^\d+$/.test(value)) {
...
function isTimestampEligibleUI(timestamp: number | null, now: number, config: UIDeleteConfig): boolean {
  if (config.mode === "all") {
    return true;
  }
  if (timestamp === null) {
...
function isNsfwUI(element: Element): boolean {
  const attrNames = ["nsfw", "data-nsfw", "data-is-nsfw", "data-subreddit-nsfw", "data-over-18", "over18"];
  if (hasTruthyAttrUI(element, attrNames)) return true;
  if (element.classList.contains("over18") || element.classList.contains("nsfw")) return true;

  const ancestor = element.closest(".over18, .nsfw, [data-over-18], [data-nsfw], [data-is-nsfw], [data-subreddit-nsfw], [over18]");
  if (ancestor && hasTruthyAttrUI(ancestor, attrNames)) return true;
  if (ancestor && (ancestor.classList.contains("over18") || ancestor.classList.contains("nsfw"))) return true;
...
function hasTruthyAttrUI(element: Element | null, names: string[]): boolean {
...
function isTruthyStringUI(value: string): boolean {
  const normalized = value.trim().toLowerCase();
  return normalized === "true" || normalized === "1" || normalized === "yes" || normalized === "on";
}

async function waitForItemsUI(timeoutMs: number = 8000, intervalMs: number = 400): Promise<void> {
...
function extractSubreddit(element: Element): string | null {
  ...
  if (fullname && fullname.startsWith("t5_")) {
    ...
    if (val) return val.replace(/^r\//i, "");
  }
  const subredditLink = element.querySelector("a[data-click-id='subreddit'], a[href*='/r/']") as HTMLAnchorElement | null;
...
function unitToMsUI(unit: UITimeUnit): number {
  switch (unit) {
    case "minutes":
      return 60 * 1000;
    case "hours":
