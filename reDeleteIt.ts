...
type DeleteMode = "all" | "older" | "newer";
...
let runtimeConfig: DeleteConfig | null = null;
let csrfToken: string | null = null;

// For debugging after config is set
(window as any).deleteNext = () => {
...
(async () => {
  ...
  let config: DeleteConfig | null = null;
...
async function deleteNext(config: DeleteConfig, token: string): Promise<boolean> {
...
async function graphqlPostRequest(body: string): Promise<Response> {
...
function findNextTarget(config: DeleteConfig): { element: Element, operation: string, input: Object } | null {
...
function buildTarget(element: Element): { element: Element, operation: string, input: Object } | null {
...
function promptForConfig(): DeleteConfig | null {
...
function normalizeMode(input: string): DeleteMode | null {
...
function normalizeUnit(input: string): TimeUnit | null {
  const value = input.trim().toLowerCase();
...
function extractTimestamp(element: Element): number | null {
...
function isTimestampEligible(timestamp: number | null, now: number, config: DeleteConfig): boolean {
  if (config.mode === "all") {
    return true;
  }
  if (timestamp === null) {
...
function isNsfw(element: Element): boolean {
  const attrNames = ["nsfw", "data-nsfw", "data-is-nsfw", "data-subreddit-nsfw", "data-over-18", "over18"];
  if (hasTruthyAttr(element, attrNames)) return true;
  ...
  if (ancestor && hasTruthyAttr(ancestor, attrNames)) return true;
...
function hasTruthyAttr(element: Element | null, names: string[]): boolean {
...
function saveConfig(config: DeleteConfig): void {
  sessionStorage.setItem(SESSION_CONFIG_KEY, JSON.stringify(config));
}

function loadSavedConfig(): DeleteConfig | null {
  try {
    const raw = sessionStorage.getItem(SESSION_CONFIG_KEY);
    if (!raw) return null;
    return JSON.parse(raw) as DeleteConfig;
  } catch {
    return null;
  }
}
...
function clearRunCount(): void {
  sessionStorage.removeItem(RUN_COUNT_KEY);
}

function logDebugElement(element: Element, reason: string): void {
...
async function waitForItems(timeoutMs: number = 8000, intervalMs: number = 400): Promise<void> {
